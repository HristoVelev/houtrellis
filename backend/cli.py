...
def poll_task_progress(endpoint, task_id, log_file):
    ...
    print("Tailing Server Log Stream:")

    while True:
        if time.time() - start_time > timeout:
            print(f"\nError: Task {task_id} timed out after {timeout} seconds.")
            sys.exit(1)
        ...
        if status == "completed":
            ...
            print(f"\n❌ Task Failed on Server: {data.get('error')}")
            sys.exit(1)
...
def handle_txt2img(args):
    ...
    # Poll and tail
    result = poll_task_progress("txt2img", data["task_id"], data["log_path"])

    # Copy output to desired location if set
...
def handle_img23d(args):
    ...
    if not os.path.exists(args.image_path):
        print(f"Error: Specified reference image not found: {args.image_path}")
        sys.exit(1)
    ...
    # Poll and tail
    result = poll_task_progress("img23d", data["task_id"], data["log_path"])
...
def main():
    ...
    parser = argparse.ArgumentParser(description="HouTrellis Command Line Tool Suite")
    ...
    if args.command == "run":
        run_yaml_pipeline(args.yaml_path)
    elif args.command == "txt2img":
        handle_txt2img(args)
    elif args.command == "img23d":
        handle_img23d(args)
    else:
        parser.print_help()
